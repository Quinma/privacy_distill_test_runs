...
for ref in c1 c3; do
  ...
  ref_target_json="$REF_EVAL_DIR/${ref}_student.json"
...
"$PY" - \
  "$PLACEBO_TARGET_JSON" \
  "$REF_EVAL_DIR/c3_student.json" \
  "$REF_EVAL_DIR/c1_student.json" \
  "$PLACEBO_RETAIN_JSON" \
...
echo "Neo placebo C6 NPO control complete under $OUT_ROOT"
