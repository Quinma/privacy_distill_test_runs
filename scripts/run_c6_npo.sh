...
BASELINE_C1_UTILITY_JSON="${BASELINE_C1_UTILITY_JSON:-$ROOT/outputs/mia/utility_c1_teacher.json}"
...
log "C6 NPO start: model=$MODEL student=$STUDENT seed=$SEED npo_beta=$NPO_BETA retain_weight=$RETAIN_WEIGHT"
...
if ! model_ready "$UNLEARN_OUT"; then
  if [[ "$UNLEARN_NPROC" -gt 1 ]]; then
    ...
    run_logged "unlearn_c6_npo" \
      "PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CUDA_VISIBLE_DEVICES=$VISIBLE_GPUS $PY -m torch.distributed.run --nproc_per_node=$UNLEARN_NPROC $ROOT/src/unlearn_teacher.py --model '$POLICY_INIT' --ref-model '$REF_MODEL' --forget-dataset '$FORGET_DATA' --retain-dataset '$RETAIN_DATA' --output '$UNLEARN_OUT' --method npo --npo-beta $NPO_BETA --beta $RETAIN_WEIGHT --epochs $UNLEARN_EPOCHS --lr $UNLEARN_LR --optim adamw --batch-size $PER_DEVICE_BATCH --grad-accum $GRAD_ACCUM --seed $SEED --fsdp $CPU_OFFLOAD_FLAG $(bf16_flag)"
  else
    run_logged "unlearn_c6_npo" \
