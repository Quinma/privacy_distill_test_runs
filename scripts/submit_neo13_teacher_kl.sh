...
CUDA_VISIBLE_DEVICES=0 "$PY" "$REPO_ROOT/scripts/teacher_kl_diagnostic.py" \
  --c6-model "$OUT/teachers/c6_unlearn" \
  --c1-model "$OUT/teachers/c1" \
  --c3-model "$OUT/teachers/c3" \
  --target-dataset "$TARGET" \
  --retain-dataset "$RETAIN" \
  --output "$ATTACK_DIR/teacher_kl_targets_retains.json" \
  --batch-size "${TEACHER_KL_BATCH_SIZE:-2}" \
  --max-length "${TEACHER_KL_MAX_LENGTH:-512}" \
  --max-chars "${TEACHER_KL_MAX_CHARS:-32768}"
