...
ssh "${SSH_OPTS[@]}" "$REMOTE_HOST" "REMOTE_ROOT='$REMOTE_ROOT' bash -s" > "$STAGING_DIR/neo13_teacher_retain_remote_status.txt" <<'REMOTE'
set -euo pipefail
...
echo
echo "== teacher retain attack files =="
find outputs/gpt-neo-1.3b-local -maxdepth 3 -type f \
  \( -name 'mia_c6_teacher_deletion_attack_target_vs_retain*.json' -o -path '*/mia_teacher_attack/*teacher_*.json' \) \
  | sort || true

echo
echo "== teacher retain logs =="
find logs outputs/logs -maxdepth 3 -type f \( -name '*neo13-tdel*' -o -name '*tdel*' \) | sort || true
REMOTE

rsync "${RSYNC_OPTS[@]}" -e "$RSYNC_RSH" \
  "$REMOTE_HOST:$REMOTE_ROOT/outputs/gpt-neo-1.3b-local/mia_c6_teacher_deletion_attack_target_vs_retain_c3ref.json" \
  "$STAGING_DIR/outputs/gpt-neo-1.3b-local/" || true

rsync "${RSYNC_OPTS[@]}" -e "$RSYNC_RSH" \
  "$REMOTE_HOST:$REMOTE_ROOT/outputs/gpt-neo-1.3b-local/mia_c6_teacher_deletion_attack_target_vs_retain_c1ref.json" \
  "$STAGING_DIR/outputs/gpt-neo-1.3b-local/" || true

rsync "${RSYNC_OPTS[@]}" -e "$RSYNC_RSH" \
  "$REMOTE_HOST:$REMOTE_ROOT/outputs/gpt-neo-1.3b-local/mia_teacher_attack/" \
  "$STAGING_DIR/outputs/gpt-neo-1.3b-local/mia_teacher_attack/" || true

mkdir -p "$STAGING_DIR/logs" "$STAGING_DIR/outputs/logs"
rsync "${RSYNC_OPTS[@]}" --include='*/' --include='*neo13-tdel*' --include='*tdel*' --exclude='*' -e "$RSYNC_RSH" \
