...
def build_result(placebo_target, ref_target, placebo_retain, ref_retain, reference: str, seed: int) -> dict:
...
def main() -> int:
    ...
    parser.add_argument("--placebo-target", required=True)
    parser.add_argument("--ref-target", required=True)
    parser.add_argument("--placebo-retain", required=True)
    ...
    result = build_result(
        load_company_losses(Path(args.placebo_target)),
        load_company_losses(Path(args.ref_target)),
        load_company_losses(Path(args.placebo_retain)),
