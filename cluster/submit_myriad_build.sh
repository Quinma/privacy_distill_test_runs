...
VARS="MODEL=$MODEL,STUDENT=$STUDENT,RUN_TAG=$RUN_TAG,STAGE=pipeline_full,REUSE_DATASETS=0,SKIP_TRAIN=1,TRAIN_DDP_NPROC=$GPUS,DISTILL_DDP_NPROC=$GPUS,UNLEARN_FSDP_NPROC=$GPUS,VISIBLE_GPUS=$VISIBLE_GPUS,REPO_ROOT=$ROOT"
